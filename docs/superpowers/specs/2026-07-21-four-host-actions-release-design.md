...
`quality` 先运行格式、静态检查、单元测试和 Race Test。`prepare-runtime-sources` 从唯一固定官方 URL 下载一次 QEMU 源码、校验 SHA-256 并上传当前 Run Artifact。五个 Runtime Job 分别重新校验该归档后并行构建；每个平台产品 Job 同时依赖自己的 Host Runtime 和公共 Guest Runtime。`aggregate` 只依赖四个平台产品 Job。`publish-rc` 只在严格 RC Tag 事件且 `aggregate` 成功时运行。
...
4. 下载当前 Workflow Run 内由 `prepare-runtime-sources` 生成的 QEMU v11.0.2 源码 Artifact，并再次校验 SHA-256。
...
Darwin 依赖许可证文本固定在 `runtime/host/darwin-licenses`，来源是 Build Lock 中已校验源码归档内声明的精确文件。两个 Darwin Build Lock 对每个许可证文件单独记录 SHA-256；打包器复制后重新计算哈希，不匹配立即失败。QEMU 自身许可证直接来自当前 Run 已校验并解开的 QEMU 源码。CI 不在打包阶段再次下载 GLib、Pixman、libslirp、zstd、gettext 或 PCRE2 源码。
